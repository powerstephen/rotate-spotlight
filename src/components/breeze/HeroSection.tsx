...
type Signal = { type: "info" | "alert" | "queued"; text: string; };
...
export function HeroSection() {
  ...
  if (typing) {
    visible.push({ id: typing.id, type: typing.signal.type, text: typing.signal.text, typing: true, partialText: typing.signal.text.slice(0, typing.chars) });
  }
  ...
  return (
    <section className="w-full px-6 py-16 md:py-20" style={{ background: "var(--bg-deep)" }}>
      <style>{`
        ...
        @keyframes hero-rec-pulse { 0%,100%{opacity:1;transform:scale(1)}50%{opacity:0.5;transform:scale(0.8)} }
      `}</style>
      <div className="mx-auto max-w-7xl">
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-14">
          ...
          <div>
            ...
            <h1 style={{ color: "var(--ink)", fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: "clamp(2rem, 3.5vw, 2.75rem)", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-0.02em" }}>
              Your best customers, your biggest risks, your next deal.{" "}
              <br />
              <span style={{ color: "var(--agent-mint)" }}>It's all in your data.</span>
              <br />
              <span style={{ color: "var(--agent-mint)" }}>SignalOps unlocks it.</span>
            </h1>
            <p className="mt-5 text-base leading-relaxed" style={{ color: "var(--agent-text-muted)", fontFamily: "'Plus Jakarta Sans', sans-serif", maxWidth: "480px" }}>
          ...
          <div className="relative flex flex-col items-center">
            ...
            <div className="mb-1.5 flex h-3.5 w-12 items-center justify-center rounded-full"
              ...
              <span className="h-2 w-2 rounded-full"
                style={{ background: "oklch(0.78 0.19 145)", boxShadow: "0 0 8px oklch(0.78 0.19 145 / 0.9), 0 0 16px oklch(0.78 0.19 145 / 0.5)", animation: "hero-rec-pulse 1.4s ease-in-out infinite" }} />
            </div>

            {/* Outer metallic frame — lighter grey */}
            <div className="relative w-full rounded-2xl p-[2.5px]"
              style={{
                ...
                boxShadow: "0 0 0 1px oklch(0.82 0.14 175 / 0.08), 0 0 40px oklch(0.82 0.14 175 / 0.15), 0 24px 60px oklch(0 0 0 / 0.6)",
              }}>

              {/* Inner bezel — lighter */}
              <div className="rounded-[13px] p-2.5"
                style={{
                  background: "linear-gradient(180deg, oklch(0.42 0.008 235) 0%, oklch(0.32 0.008 235) 100%)",
                  boxShadow: "inset 0 2px 4px oklch(0 0 0 / 0.3), inset 0 0 0 1px oklch(1 0 0 / 0.06)",
                }}>

                {/* Screen */}
                <div className="rounded-xl overflow-hidden" style={{ background: "var(--gradient-screen)", boxShadow: "var(--inset-screen)", minHeight: "320px", padding: "16px 20px" }}>
                  ...
                  <div className="flex items-center gap-2 mb-4 pb-3" style={{ borderBottom: "1px solid oklch(0.32 0.04 200 / 0.2)" }}>
                    ...
                    <span className="ml-3 font-mono text-[10px] uppercase tracking-[0.2em]" style={{ color: "var(--agent-mint-dim)" }}>
                      SignalOps · Live Signal Feed
                    </span>
                  </div>

                  {/* Signal lines — full width */}
                  <div className="space-y-2 font-mono text-[11px] leading-relaxed w-full">
                    {trimmed.map((line) => (
                      <div key={line.id} style={{ color: colorFor(line.type), filter: glowFor(line.type) }}>
                        {line.typing ? line.partialText : line.text}
                        {line.typing && (
                          <span aria-hidden style={{ display: "inline-block", marginLeft: "2px", animation: "hero-cursor-blink 1s steps(1) infinite" }}>_</span>
                        )}
                      </div>
                    ))}
                  </div>
            ...
            {/* Neck */}
            <div style={{ width: "26px", height: "20px", background: "linear-gradient(180deg, oklch(0.45 0.008 235), oklch(0.32 0.008 235))", boxShadow: "inset 0 1px 0 oklch(1 0 0 / 0.08)" }} />

            {/* Base */}
            <div style={{ width: "140px", height: "7px", borderRadius: "4px", background: "linear-gradient(180deg, oklch(0.5 0.008 235), oklch(0.35 0.008 235))", boxShadow: "inset 0 1px 0 oklch(1 0 0 / 0.1), 0 4px 16px oklch(0 0 0 / 0.5)" }} />
          </div>
        </div>
      </div>
    </section>
